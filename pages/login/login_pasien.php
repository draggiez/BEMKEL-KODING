...
<body class="hold-transition login-page">
  ...
  if (isset($_POST['submit'])) {
    ...
    } elseif (count((array) $conn->query('SELECT email FROM pasien WHERE email = "' . $email . '"')->fetch_array()) == 0) {
      echo '<b>Warning!</b> Email tidak terdaftar.';
    } else {
      $result = $conn->query("SELECT * FROM pasien WHERE email='$email' AND password='$password'");
      if ($result->num_rows == 1) {
        $pasien = $conn->query("SELECT id,nama,no_rm FROM pasien WHERE email='$email'")->fetch_assoc();
        ...
        header('Location: ../dashboard/db_pasien.php'); // Alihkan ke halaman index.
        exit();
      }
    }
  }

...
<body class="hold-transition login-page">
  ...
  if (isset($_SESSION['nama_dokter'])) {
    header('Location: ../dashboard/db_dokter.php');
    exit();
  }
  if (isset($_POST['submit'])) {
    ...
    } elseif (count((array) $conn->query('SELECT email FROM dokter WHERE email = "' . $email . '"')->fetch_array()) == 0) {
      echo '<b>Warning!</b> Email tidak terdaftar.';
    } else {
      $result = $conn->query("SELECT * FROM dokter WHERE email='$email' AND password='$password'");
      if ($result->num_rows == 1) {
        $dokter = $conn->query("SELECT id,nama FROM dokter WHERE email='$email'")->fetch_assoc();
		...
		header('Location: ../dashboard/db_dokter.php'); // Alihkan ke halaman index.
        exit();
      }
    }
  }
  ...
  <div class="login-box">
    <div class="login-logo">
      <b>Dokter</b><br> Rumah Sakit Bengkod
    </div>
    <!-- /.login-logo -->
    <div class="card">
